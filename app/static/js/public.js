...
(function () {
	...
	function initAddressAutocomplete(inputEl, options) {
		if (!inputEl || inputEl.dataset.addressAutocompleteBound === "1") {
			return;
		}
		...
		var opts = Object.assign(
			{
				apiUrl: "/tenant/api/address-autocomplete",
				minLength: 3,
				debounceMs: 350,
			},
			options || {}
		...
		dropdown.className = "list-group position-absolute w-100 shadow-sm border-0";
		dropdown.style.cssText =
			"z-index: 1055; top: 100%; left: 0; max-height: 220px; " +
		...
		function fetchSuggestions(q) {
			var url = opts.apiUrl + "?q=" + encodeURIComponent(q);
